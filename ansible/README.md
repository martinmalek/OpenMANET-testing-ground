...
**Note:** The TAK Server ZIP file must be downloaded from [tak.gov/products/tak-server](https://tak.gov/products/tak-server) and placed on your local machine. The role will transfer it to the device automatically. If `tak_server_run_setup` is `false`, you'll need to run `setup.sh` manually on the device.
...
**Variables** (defined in `roles/opentakserver-dted/defaults/main.yml`):
...
**Note:** This role runs on `localhost` (the control machine) and requires `curl` and `python3` to be installed. It downloads files from GitHub and uploads them to OpenTAKServer via the API.
...
**Variables** (defined in `roles/opentakserver-packages/defaults/main.yml`):
- `opentakserver_host`: OpenTAKServer hostname or IP (default: `{{ ansible_host | default(inventory_hostname) }}`)
- `opentakserver_port`: OpenTAKServer port (default: `8880`)
- `opentakserver_username`: Username for authentication (default: `administrator`)
- `opentakserver_password`: Password for authentication (default: `password`)
...
**Note:** This role requires administrator privileges. Only `.apk` files are supported by the API. `.xapk` files (which are ZIP archives) need to be extracted first before uploading.

**Reference:** [OpenTAKServer Package API](https://github.com/brian7704/OpenTAKServer/blob/master/opentakserver/blueprints/ots_api/package_api.py)
